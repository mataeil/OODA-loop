...
| **Safety Levels** | Progressive autonomy tiers. **Level 0**: observe only, 1 domain. **Level 1**: observe + test, 2 domains. **Level 2**: full observation, all domains, draft PRs (human merges). **Level 3**: autonomous, implementation enabled, low-risk auto-merge available as an opt-in (`safety.enable_auto_merge`, default off — Draft PRs you merge until you turn it on). |
...
| **Outcome Record** (v1.4.0) | Per-cycle "did this help?" signal in `agent/state/evolve/outcomes.json` (evolve Step 6-C9). Deterministic — scores each cycle a `quality_multiplier` (0.0–1.0) from its actual `result_type` (pr_merged_held 1.0 … futile 0.0). The atomic ground truth distinguishing "ran" from "helped". Reference: `scripts/score_outcome.py`. |
| **quality_multiplier** (v1.4.0) | A cycle's outcome score, 0.0–1.0, from the Step 6-C9 table. Back-annotated upward as a PR merges and holds (2-B4), or to 0.0 on revert/reject. The mean across cycles is the **Loop Value Score**. |
| **Loop Scorecard** (v1.4.0) | `/ooda-status --scorecard` — the loop-engineering measurement view: Loop Value Score, Task Completion Rate, Futile Cycle Rate, PR merge & hold, Queue Resolution, Cost/Success, Goal Progress, learning-loop health. Reference: `scripts/loop_scorecard.py`. Answers "is the loop working?", not just "did it run?". |
| **Maker/Checker eval** (v1.4.0) | Opt-in `config.eval`: a *separate* model independently grades whether a cycle met its declared goal (evolve Step 7-B), written to `outcomes.json.verifier_verdict`. The deterministic quality_multiplier stays ground truth; the verdict is a second opinion (the writer never grades its own work). |
| **Verifiable goal** (loop-engineering canon #1) | A `goals.json` entry with a `metric_command` (a shell command printing a number/boolean). Step 2-C drives `goal.progress` from it; the Scorecard shows Goal Progress. The machine-checkable done-condition a loop should be written *toward* before it starts. |
...
| **Cycle Card** (v1.2.0) | A screenshottable end-of-cycle artifact rendered by evolve Step 7, gated by `config.output.cycle_card` (default `true`; skipped in `--dry-run`, which exits at Step 3-H). It composes Observe → Orient → Decide → Act → **Learn** → Cost from data the cycle already wrote — no new computation. The **LEARN** line is the differentiator: it surfaces the single highest-signal re-orientation in priority order — a confidence change from a human merge/reject > a lens re-aim (sourced from `lens_changelog.json`) > a new intervention > an observation micro-adjustment. `/ooda-status --share` re-renders the latest card read-only. Framing is deliberately honest (outcome-driven re-orientation; the rendered verbs are "re-aimed / adjusted / deprioritized", never "trained" or "learned weights"). |
| **Cycle Card Output Toggle** (v1.2.0) | `config.output.cycle_card` (default `true`). When `false`, evolve suppresses the Step 7 Cycle Card. The card is always re-renderable on demand via `/ooda-status --share` regardless of this toggle. |
| **Reflexion Loop / Verbal Self-Critique** (v1.2.0) | A [Reflexion](https://arxiv.org/abs/2303.11366)-style learning mechanism. evolve Step 5-F writes a short natural-language self-critique of each decision cycle — `{verdict: hit/miss/too_early/unclear, lesson}` — to `agent/state/evolve/reflections.json` (capped by `memory.reflections_buffer_size`, default 20). Step 2-F recalls up to `memory.reflection_recall_count` (default 3) relevant recent lessons and re-injects them into the next Orient (informing the orient_summary and breaking near-ties in Decide). It is verbal self-correction stored as language and re-read next cycle — explicitly NOT weight updates, training, or RL. This is the honest, inspectable form of "the agent learns from its own cycles," and the most credible SOTA-adjacent claim the harness can make. Visible in `/ooda-status` Orient Health (`Reflections` count + latest lesson). |
