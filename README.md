...
> **Loop engineering, with a scoreboard.** 2026's shift is from *prompting* an agent to *designing the loop that runs it until a verifiable goal is met*. OODA-loop is that loop — Observe→Orient→Decide→Act→Reflect on a schedule. At `/ooda-setup` you **state the project's mission once**; from then on the loop self-drives toward it — Decide is pulled toward mission-aligned work, off-mission distractions are deprioritized, and when the goal is met it **idles instead of spinning**. And it's the rare loop that **measures whether it's actually working**: every cycle gets a `quality_multiplier`, and `/ooda-status --scorecard` gives you a Loop Value Score, mission-hit rate, futile-cycle rate, and an A–F grade. A loop you can't measure is just an open invoice.
>
> *These behaviours were tuned empirically — 8 measured iterations against three simulated projects (`tests/sim/`), each change kept only if the scorecard improved (`tests/sim/RESULTS.md`). Mission-awareness alone lifted goal completion +14–40pp.*
